...
gltkGeometry LoadOFFFromFile(std::string const& filename)
{
  ...
  std::vector<cpe::vec3> v_vertices;
  std::vector<int> v_connectivity;

  std::ifstream fid(filename.c_str());
  if(!fid.good())
    throw cpe::exception_cpe("Cannot open file "+filename,EXCEPTION_PARAMETERS_CPE);
  ...
  {
    if(!fid.good())
      throw cpe::exception_cpe("Cannot find OFF header in file "+filename,EXCEPTION_PARAMETERS_CPE);
  ...
  for(int k=0;k<N_triangle;++k)
  {
    ...
    if(N_size!=3)
      throw cpe::exception_cpe("Cannot read OFF with non triangular faces for file "+filename,MACRO_EXCEPTION_PARAMETER);
  ...
  if(v_connectivity.size()%3!=0)
    throw cpe::exception_cpe("Problem with size of connectivity in file "+filename,EXCEPTION_PARAMETERS_CPE);
  ...
  for(int k=0,N=v_connectivity.size()/3;k<N;++k)
  {
    ...
    m.AddTriangleIndex(cpe::triangle_index(u0,u1,u2));
  }
  ...
  ASSERT_CPE(m.IsValid(),"Mesh "+filename+" is invalid");
